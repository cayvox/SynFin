...
- If `networkFee.asset == intent.give.asset`: `worstCaseReceiveNet = floor( gross * give / (give + fee) , receiveDecimals )`. This re-bases the receipt to "received per unit of total give-asset outlay," which is exact (no rate conversion) and floors in the taker's favor. It equals the algebraic identity `gross - fee * (gross / give)`, i.e. the fee valued at the plan's own average rate and subtracted, which is the 1inch gas-aware net.
